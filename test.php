    if(isset($_FILES['hinhanhlienquan3']))
    {
      ...
      if(!$file_name3=='')
      {
      $check = getimagesize($_FILES["hinhanhlienquan3"]["tmp_name"]);
      if($check !== false) {
      ...
      if ($_FILES["hinhanhlienquan3"]["size"] > 500000) {
        echo "<script>alert('Kích thước ảnh mô tả 3 quá lớn')
        location.href = 'dangsanpham';
        </script>";
        $uploadOk = 0;
        }
      ...
      $newname3= $rename.$file_name;
      if($file['type'] == 'image/jpeg' || $file['type'] == 'image/jpg' || $file['type'] == 'image/png')
      {
        move_uploaded_file($file['tmp_name'],'data/sanpham/'.$newname3);
      }
      else{
